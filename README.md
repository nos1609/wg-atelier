...
**wg-atelier** — репозиторий утилиты `generate_wg_configs.py`, которая собирает полный набор WireGuard-конфигов на основе описания сервера (`wg_config.yaml`), списка подсетей (`subnets.csv`) и уже существующих клиентских профилей. Скрипт переиспользует приватные ключи, следит за PSK и формирует как серверный файл `server/wg0.conf`, так и клиентские профили с QR-кодами.

## Возможности

- чтение параметров сервера и AmneziaWG из YAML;
- автоматическая сверка CSV с содержимым `clients/<subnet>/` и дозапрос действий при расхождениях;
- сохранение серверного и клиентских конфигов, генерация QR-кодов;
- логирование в `wg-atelier.log` и создание резервных копий `wg_config.yaml.<timestamp>.bak` при автозаписи ключей;
- интерактивная работа с PSK (подтягивание существующих значений, подтверждения перед регенерацией);
- режим `--validate` для «сухой» проверки конфигурации без записи файлов.

## Требования

### Внешние CLI
- `wg` (`wireguard-tools`) — генерация ключей, получение public key, создание PSK.
- `wireguard-vanity-address` — требуется только при `vanity_length > 0`.
- Скрипт запускается под Python 3.10\+ на Windows.

### Python
- Python 3.10+.
- Зависимости фиксируются в `requirements.txt` (установите `pip install -r requirements.txt`).
...
- порядок строк определяет порядок подсетей в выходных файлах.

### Существующие клиенты
- `clients/<prefix>/<prefix>_clientN.conf` используются как источник приватных ключей/PSK.
- при удалении строки из CSV скрипт спросит, нужно ли удалить каталог подсети.

## Запуск

1. Подготовьте `wg_config.yaml` и `subnets.csv`, при необходимости очистите лишние каталоги в `clients/`.
2. Установите зависимости (`pip install -r requirements.txt`) и убедитесь, что `wg`/`wireguard-vanity-address` доступны в PATH.
3. Запустите генератор:
   ```
   python generate_wg_configs.py -c config/wg_config.yaml -s data/subnets.csv -o ./artifacts
   ```
   Для «сухой» проверки без записи файлов: `python generate_wg_configs.py --validate`.
4. Следуйте интерактивным подсказкам (подтверждение удаления подсетей, перенос PSK, чтение существующих клиентов).

| Ключ | Описание |
| --- | --- |
| `-c/--config` | Путь к `wg_config.yaml` |
...
## PSK, AmneziaWG и keepalive

- `psk: generate` — создаётся общий PSK, после чего режим автоматически переключается на `static`.
- `psk: generate_per_client` — для каждого клиента генерируется отдельный PSK.
- `psk: static` + `value` — используется готовое значение (строка Base64, 32 байта). При отсутствии значения можно подтянуть PSK из существующих клиентов.
- `psk.reuse=true` заставляет скрипт переиспользовать найденный PSK, `false` — требует явного согласия перед регенерацией.
...
- При `vanity_length=0` скрипт автоматически переключается на стандартную генерацию `wg genkey`.
- Максимальная длина префикса — 10 символов (ограничение встроено в скрипт).

## Логи и диагностика

- Основной лог: `wg-atelier.log`.
- При прерывании Ctrl+C ловится `KeyboardInterrupt`, запись попадает в лог.
- Для анализа проблем полезно запускать `python -m py_compile generate_wg_configs.py` и `python generate_wg_configs.py --validate` перед публикацией.

## Безопасность

- Приватные ключи и конфиги не должны покидать локальную машину.
- Скрипт выставляет права `0600` на конфиги (на Windows — через `icacls`, отключая наследование и выдавая права текущему пользователю). Проверяйте, что каталоги недоступны посторонним.
